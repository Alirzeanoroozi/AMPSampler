...
if boltz1:
    ...
    if os.path.exists(plddt_file_path):
        ...
        raw_plddt = np.array(data_plddt["plddt"])
        # Boltz may store pLDDT in [0,1] or already in [0,100]
        plddt_boltz1 = 100.0 * raw_plddt if np.nanmax(raw_plddt) <= 1.0 else raw_plddt
        # Newer Boltz writes residue-level arrays (len == numres); older atom-token arrays need CA mask
        if plddt_boltz1.shape[0] == numres:
            plddt = plddt_boltz1
            cb_plddt = plddt_boltz1
        else:
            sel = token_array.astype(bool)
            plddt = plddt_boltz1[sel]
            cb_plddt = plddt_boltz1[sel]
    else:
    ...
    if os.path.exists(pae_file_path):
        ...
        if pae_matrix_boltz1.shape[0] == numres:
            ...
            sel = token_array.astype(bool)
            pae_matrix = pae_matrix_boltz1[sel][:, sel]
    else:
        ...
        with open(summary_file_path, "r") as file:
            ...
            for chain1 in unique_chains:
                ...
                for chain2 in unique_chains:
                    ...
                    iptm_boltz1[chain1][chain2] = boltz1_chain_pair_iptm_data[str(nchain1)][
                        str(nchain2)
                    ]
    else:

...
Runs in the base conda env. Usage (from AMPBinderDesign):
  python src/4_structure_prediction/active_site_overlap.py --target NDM5
  python src/4_structure_prediction/active_site_overlap.py --target KPC3 \\
      --complexes boltz_results/boltz_results_boltz_inputs/predictions
"""
...
_DEFAULT_COMPLEXES = os.path.join(
    "boltz_results", "boltz_results_boltz_inputs", "predictions"
)
...
def list_complexes(path: str, target: str | None = None) -> list[str]:
    if os.path.isfile(path):
        return [path]
    hits: list[str] = []
    for ext in ("*.cif", "*.pdb"):
        hits.extend(glob.glob(os.path.join(path, "**", ext), recursive=True))
    model0 = [h for h in hits if re.search(r"_model_0\.(cif|pdb)$", h, re.I)]
    files = model0 or hits
    out = []
    for f in files:
        parts = set(os.path.normpath(f).split(os.sep))
        if parts & _SKIP_DIR_PARTS:
            continue
        did = _design_id_from_path(f)
...
import src.structure_io as sio  # noqa: E402
...
def identify_chains(structure, design_seq):
    ...
    for chain in model:
        ...
        from src.structure_io import _aligner
...
def main():
    ...
    ap.add_argument(
        "--complexes",
        default=os.path.join(REPO, _DEFAULT_COMPLEXES),
        help="binder-target complex file or directory (searched recursively for *_model_0.cif)",
    )
    ...
    files = list_complexes(args.complexes, target=tkey)
    if not files:
        print(f"No complex files found under {args.complexes} for {tkey}.")
        return 1
    ...
    out = args.out or os.path.join(REPO, "results", f"active_site_overlap_{tkey}.csv")
